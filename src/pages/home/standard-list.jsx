            ...
            },
            {
                ...
                render: (text, record) => {
                    if (record.title.length < 15) {
                        return <Meta
                            avatar={<Avatar src={require("./images/bootstrap.png")} style={{ width: 30, height: 30 }} />}
                            title={<Title level={5}>{record.title}</Title>}
                            description={record.content}
                        />
                    } else if (record.title.length >= 15 && record.title.length < 20) {
                        return <Meta
                            avatar={<Avatar src={require("./images/react.png")} style={{ width: 30, height: 30 }} />}
                            title={<Title level={5}>{record.title}</Title>}
                            description={record.content}
                        />
                    } else if (record.title.length >= 20 && record.title.length < 25) {
                        return <Meta
                            avatar={<Avatar src={require("./images/alipay.png")} style={{ width: 30, height: 30 }} />}
                            title={<Title level={5}>{record.title}</Title>}
                            description={record.content}
                        />
                    } else if (record.title.length >= 25 && record.title.length < 30) {
                        return <Meta
                            avatar={<Avatar src={require("./images/pro.png")} style={{ width: 30, height: 30 }} />}
                            title={<Title level={5}>{record.title}</Title>}
                            description={record.content}
                        />
                    } else {
                        return <Meta
                            avatar={<Avatar src={require("./images/ai.png")} style={{ width: 30, height: 30 }} />}
                            title={<Title level={5}>{record.title}</Title>}
                            description={record.content}
                        />
                    }
